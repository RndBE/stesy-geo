...
function projectGuard(req: Request, res: Response, projectId: number): boolean {
  if (!req.user || !canAccessProject(req.user, projectId)) { res.status(403).json({ error: 'Tidak punya akses ke proyek ini' }); return false; }
  return true;
}
function instGuard(req: Request, res: Response): Instrument | null {
  const i = getInstrument(Number(req.params.id));
  if (!i) { res.status(404).json({ error: 'Instrumen tidak ditemukan' }); return null; }
  return projectGuard(req, res, i.project_id) ? i : null;
}
function zoneGuard(req: Request, res: Response) {
  const z = getZone(Number(req.params.id));
  if (!z) { res.status(404).json({ error: 'Zona tidak ditemukan' }); return null; }
  return projectGuard(req, res, z.project_id) ? z : null;
}

// ---------------------------------------------------------------- auth & ingest (tanpa sesi pengguna)

api.post('/login', wrap((req, res) => {
  const r = login(String(req.body?.email ?? ''), String(req.body?.password ?? ''));
  if (!r) return res.status(401).json({ error: 'Email atau kata sandi salah' });
  audit(r.user.id, 'session', r.user.id, 'login', null, null);
  res.json(r);
}));

/** F-TLM-03: HTTP push dari gateway dengan token Bearer per gateway. */
api.post('/ingest', wrap((req, res) => {
  const token = req.headers.authorization?.replace(/^Bearer /, '');
  const gw = authenticateGateway(req.body?.gateway, token);
  if (!gw) return res.status(401).json({ error: 'Token gateway tidak valid' });
  res.json(ingestPayload(req.body, gw.id));
}));
...
api.get('/projects', wrap((req, res) => {
  const rows = db().prepare('SELECT * FROM project ORDER BY id').all() as any[];
  res.json(rows.filter((p) => canAccessProject(req.user!, p.id)));
}));

api.get('/projects/:id/overview', wrap((req, res) => {
  const pid = Number(req.params.id);
  if (!projectGuard(req, res, pid)) return;
  const project = db().prepare('SELECT * FROM project WHERE id = ?').get(pid);
  const now = Date.now();
  const zones = getZones(pid).map((z) => {
    ...
    return {
      ...
      polygon: JSON.parse((db().prepare('SELECT polygon FROM zone WHERE id = ?').get(z.id) as any).polygon ?? '[]'),
  ...
  const insts = db().prepare('SELECT * FROM instrument WHERE project_id = ?').all(pid) as Instrument[];
  const stale = insts.filter((i) => i.status === 'aktif' && isStale(i, now)).map((i) => ({ id: i.id, code: i.code, type: i.type, last: lastReading(i.id)?.ts ?? null, mode: i.mode }));
  const latest = db().prepare(`SELECT MAX(r.ts) m FROM reading r JOIN instrument i ON i.id = r.instrument_id WHERE i.project_id = ? AND r.source = 'telemetry'`).get(pid) as any;
...
api.get('/projects/:id/trend', wrap((req, res) => {
  const pid = Number(req.params.id);
  if (!projectGuard(req, res, pid)) return;
  const days = num(req.query.days, 30)!;
  const from = Date.now() - days * DAY;
  const out = getZones(pid).map((z) => {
    const centre = zoneInstruments(z.id).filter((i) => ['SC', 'GN'].includes(i.type) && Math.abs(i.offset ?? 99) <= 6);
    const series = centre.map((i) => dailySeries(i.id, from));
...
api.get('/projects/:id/twin', wrap((req, res) => {
  const pid = Number(req.params.id);
  if (!projectGuard(req, res, pid)) return;
  res.json(buildTwin(pid, num(req.query.horizon, 180)));
}));

api.get('/projects/:id/longitudinal', wrap((req, res) => {
  const pid = Number(req.params.id);
  if (!projectGuard(req, res, pid)) return;
  const al = db().prepare('SELECT * FROM alignment WHERE project_id = ?').get(pid) as any;
  res.json({ ...longitudinal(pid), alignment: al ? { name: al.name, geometry: JSON.parse(al.sta_geometry), design: JSON.parse(al.design_profile) } : null });
}));

// ---------------------------------------------------------------- zona

api.get('/zones/:id', wrap((req, res) => {
  const z = zoneGuard(req, res);
  if (!z) return;
  const s = zoneStatus(z);
  const stages = getStages(z.id);
  const pvd = getPvd(z.id);
  const insts = zoneInstruments(z.id).map((i) => ({ ...i, meta: meta(i), last: lastReading(i.id) ?? null, stale: isStale(i), typeLabel: TYPE_LABEL[i.type] }));
  const pvdInfo = pvd ? {
    ...pvd, De: influenceDiameter(pvd), dw: drainDiameter(pvd), mu: hansboMu(pvd),
    t90_years: hansboTimeForUh(0.9, z.ch, pvd),
  } : null;
  const recs = db().prepare('SELECT r.*, u.name AS by_name FROM recommendation r LEFT JOIN app_user u ON u.id = r.created_by WHERE zone_id = ? ORDER BY created_at DESC').all(z.id);
  res.json({ status: s, stages, layers: getLayers(z.id), pvd: pvdInfo, instruments: insts, recommendations: recs, algoVersion: ALGO_VERSION });
}));

/** Grafik gabungan waktu–penurunan–tinggi timbunan zona (F-ANL-01 tingkat zona). */
api.get('/zones/:id/combined', wrap((req, res) => {
  const z = zoneGuard(req, res);
  if (!z) return;
  const stages = getStages(z.id);
  const insts = zoneInstruments(z.id).filter((i) => ['SC', 'GN', 'SP'].includes(i.type));
  const series = insts.map((i) => ({ code: i.code, type: i.type, offset: i.offset, sta: i.sta, data: dailySeries(i.id).map((p) => [p.t, +p.v.toFixed(1)]) }));
...
api.put('/zones/:id/params', requireRole('engineer'), wrap((req, res) => {
  const z = zoneGuard(req, res);
  if (!z) return;
  ...
  db().prepare(`UPDATE zone SET ${Object.keys(upd).map((k) => `${k} = ?`).join(', ')} WHERE id = ?`).run(...Object.values(upd), z.id);
  audit(req.user!.id, 'zone', z.id, 'ubah parameter', before, upd);
  res.json(getZone(z.id));
}));

api.put('/zones/:id/pvd', requireRole('engineer'), wrap((req, res) => {
  const z = zoneGuard(req, res);
  if (!z) return;
  const before = db().prepare('SELECT * FROM pvd_spec WHERE zone_id = ?').get(z.id);
  const b = req.body ?? {};
  db().prepare('UPDATE pvd_spec SET pattern = ?, spacing = ?, length = ?, a = ?, b = ?, s = ?, kh_ks = ? WHERE zone_id = ?')
    .run(b.pattern, b.spacing, b.length, b.a, b.b, b.s, b.khKs ?? b.kh_ks, z.id);
  audit(req.user!.id, 'pvd_spec', z.id, 'ubah PVD', before, b);
  res.json(getPvd(z.id));
}));

api.put('/layers/:id', requireRole('engineer'), wrap((req, res) => {
  const L = db().prepare('SELECT * FROM soil_layer WHERE id = ?').get(Number(req.params.id)) as any;
  if (!L) return res.status(404).json({ error: 'Lapisan tidak ditemukan' });
  if (!projectGuard(req, res, getZone(L.zone_id).project_id)) return;
  ...
  if (!Object.keys(upd).length) return res.status(400).json({ error: 'Tidak ada parameter valid' });
  db().prepare(`UPDATE soil_layer SET ${Object.keys(upd).map((k) => `${k} = ?`).join(', ')} WHERE id = ?`).run(...Object.values(upd), L.id);
  audit(req.user!.id, 'soil_layer', L.id, 'ubah parameter tanah', Object.fromEntries(Object.keys(upd).map((k) => [k, L[k]])), upd);
  res.json(db().prepare('SELECT * FROM soil_layer WHERE id = ?').get(L.id));
}));

api.put('/stages/:id', requireRole('engineer'), wrap((req, res) => {
  const st = db().prepare('SELECT * FROM fill_stage WHERE id = ?').get(Number(req.params.id)) as any;
  if (!st) return res.status(404).json({ error: 'Tahap tidak ditemukan' });
  if (!projectGuard(req, res, getZone(st.zone_id).project_id)) return;
  const allowed = ['planned_start', 'planned_end', 'actual_start', 'actual_end', 'thickness', 'removed_at'];
  const upd = Object.fromEntries(Object.entries(req.body ?? {}).filter(([k]) => allowed.includes(k)).map(([k, v]) => [k, v === null || v === '' ? null : Number(v)]));
  db().prepare(`UPDATE fill_stage SET ${Object.keys(upd).map((k) => `${k} = ?`).join(', ')} WHERE id = ?`).run(...Object.values(upd), st.id);
  audit(req.user!.id, 'fill_stage', st.id, 'ubah tahap timbunan', Object.fromEntries(Object.keys(upd).map((k) => [k, st[k]])), upd);
  broadcast('zones', { zone: st.zone_id });
  res.json(db().prepare('SELECT * FROM fill_stage WHERE id = ?').get(st.id));
}));

api.post('/zones/:id/recommendations', requireRole('engineer'), wrap((req, res) => {
  const z = zoneGuard(req, res);
  if (!z) return;
  const { decision, text, analysis_run_ids } = req.body ?? {};
  if (!text) return res.status(400).json({ error: 'Teks rekomendasi wajib diisi' });
  const id = db().prepare('INSERT INTO recommendation(zone_id, decision, text, analysis_run_ids, created_by, created_at) VALUES (?,?,?,?,?,?)')
    .run(z.id, decision ?? zoneStatus(z).decision, text, JSON.stringify(analysis_run_ids ?? []), req.user!.id, Date.now()).lastInsertRowid;
  audit(req.user!.id, 'recommendation', Number(id), 'buat', null, req.body);
  res.json({ id });
}));

// ---------------------------------------------------------------- instrumen & bacaan

api.get('/instruments', wrap((req, res) => {
  const pid = num(req.query.project, 1)!;
  if (!projectGuard(req, res, pid)) return;
  const rows = db().prepare('SELECT i.*, z.code AS zone_code FROM instrument i LEFT JOIN zone z ON z.id = i.zone_id WHERE i.project_id = ? ORDER BY i.type, i.code').all(pid) as any[];
  const alarm = db().prepare(`SELECT level FROM alarm_event WHERE instrument_id = ? AND cleared_at IS NULL ORDER BY CASE level WHEN 'Bahaya' THEN 3 WHEN 'Siaga' THEN 2 ELSE 1 END DESC LIMIT 1`);
  const prev = db().prepare('SELECT ts, value FROM reading WHERE instrument_id = ? ORDER BY ts DESC LIMIT 1 OFFSET 1');
  res.json(rows.map((i) => ({
    ...i, meta: meta(i), typeLabel: TYPE_LABEL[i.type], last: lastReading(i.id) ?? null, prev: prev.get(i.id) ?? null,
    stale: isStale(i), alarm: (alarm.get(i.id) as any)?.level ?? null,
  })));
}));

api.get('/instruments/:id', wrap((req, res) => {
  const i = instGuard(req, res);
  if (!i) return;
  const d = db();
  res.json({
    ...i, meta: meta(i), typeLabel: TYPE_LABEL[i.type], zone: i.zone_id ? getZone(i.zone_id) : null,
    last: lastReading(i.id) ?? null, stale: isStale(i),
    calibrations: d.prepare('SELECT * FROM calibration WHERE instrument_id = ? ORDER BY valid_from DESC').all(i.id).map((c: any) => ({ ...c, coeffs: JSON.parse(c.coeffs_json) })),
    channels: d.prepare('SELECT c.*, l.code AS logger_code, l.model FROM logger_channel c JOIN logger l ON l.id = c.logger_id WHERE c.instrument_id = ? ORDER BY valid_from DESC').all(i.id),
    alarms: d.prepare('SELECT e.*, u.name AS ack_name FROM alarm_event e LEFT JOIN app_user u ON u.id = e.ack_by WHERE e.instrument_id = ? ORDER BY e.ts DESC LIMIT 50').all(i.id),
    counts: d.prepare('SELECT source, COUNT(*) n, MIN(ts) first, MAX(ts) last FROM reading WHERE instrument_id = ? GROUP BY source').all(i.id),
  });
}));

api.get('/instruments/:id/readings', wrap((req, res) => {
  const i = instGuard(req, res);
  if (!i) return;
  ...
  if (useDaily) {
    const bySource = db().prepare('SELECT DISTINCT source FROM reading WHERE instrument_id = ?').all(i.id) as { source: string }[];
    const out: Record<string, [number, number][]> = {};
    for (const { source } of bySource) {
      const rows = db().prepare(
        `SELECT CAST((ts + 25200000) / 86400000 AS INTEGER) d, AVG(value) v FROM reading WHERE instrument_id = ? AND source = ? AND ts BETWEEN ? AND ? AND (flag IS NULL OR flag != 'ditolak') GROUP BY d ORDER BY d`,
...
api.get('/instruments/:id/table', wrap((req, res) => {
  const i = instGuard(req, res);
  if (!i) return;
...
api.get('/instruments/:id/profiles', wrap((req, res) => {
  const i = instGuard(req, res);
  if (!i) return;
  const d = db();
  const times = d.prepare('SELECT DISTINCT ts FROM reading_profile WHERE instrument_id = ? ORDER BY ts').all(i.id).map((r: any) => r.ts as number);
  ...
  const profiles = uniq.map((ts) => ({ ts, points: d.prepare('SELECT pos, value FROM reading_profile WHERE instrument_id = ? AND ts = ? ORDER BY pos').all(i.id, ts) }));
...
api.post('/instruments/:id/readings', requireRole('surveyor'), wrap((req, res) => {
  const i = instGuard(req, res);
  if (!i) return;
  try {
    const r = ingestManual(i, req.body ?? {}, req.user!.id);
    audit(req.user!.id, 'reading', r.id, 'input manual', null, { instrument: i.code, ...req.body, flag: r.flag });
    res.json(r);
...
api.post('/instruments/:id/validate', requireRole('surveyor'), wrap(async (req, res) => {
  ...
  const sc = spikeCheck(i.id, ts, v);
  res.json({ ...out, spike: sc.spike, expected: sc.expected, sd: sc.sd });
}));

api.patch('/readings/:id', requireRole('engineer'), wrap((req, res) => {
  const r = db().prepare('SELECT * FROM reading WHERE id = ?').get(Number(req.params.id)) as any;
  ...
  db().prepare('UPDATE reading SET flag = ?, note = COALESCE(?, note) WHERE id = ?').run(flag, req.body?.note ?? null, r.id);
  audit(req.user!.id, 'reading', r.id, 'ubah flag', { flag: r.flag }, { flag, note: req.body?.note });
  res.json({ ok: true });
}));

/** Import massal CSV (F-INS-03). Body: { csv, mapping: {instrument, ts, value, note?}, template? } */
api.post('/import/csv', requireRole('surveyor'), wrap((req, res) => {
  ...
  const tx = db().transaction(() => {
    for (let k = 1; k < lines.length; k++) {
      ...
      const inst = db().prepare('SELECT * FROM instrument WHERE code = ?').get(c[ci]) as Instrument | undefined;
      ...
      if (!canAccessProject(req.user!, inst.project_id)) { results.push({ line: k + 1, error: 'Tanpa akses' }); continue; }
      if (dryRun) { results.push({ line: k + 1, instrument: inst.code, ts, value, ok: true }); continue; }
      const r = ingestManual(inst, { ts, value, note: ni >= 0 ? c[ni] : 'import CSV' }, req.user!.id);
      results.push({ line: k + 1, instrument: inst.code, ts, value, flag: r.flag, ok: true });
    }
  });
  tx();
  if (!dryRun) audit(req.user!.id, 'reading', null, 'import CSV', null, { rows: results.filter((r) => r.ok).length, mapping });
...
api.get('/routes', wrap((req, res) => {
  const pid = num(req.query.project, 1)!;
  if (!projectGuard(req, res, pid)) return;
  const insts = db().prepare(`SELECT i.*, z.code AS zone_code FROM instrument i LEFT JOIN zone z ON z.id = i.zone_id WHERE i.project_id = ? ORDER BY i.sta, i.offset`).all(pid) as any[];
  const prev2 = db().prepare('SELECT ts, value, source FROM reading WHERE instrument_id = ? ORDER BY ts DESC LIMIT 3');
  const routes = [
    { id: 'kontrol', name: 'Settlement plate kontrol (waterpass)', filter: (i: any) => i.type === 'SP' },
    ...getZones(pid).map((z) => ({ id: z.code, name: `${z.code} ${z.name} — semua instrumen`, filter: (i: any) => i.zone_id === z.id && i.type !== 'SAA' && i.type !== 'INC' })),
  ];
  res.json(routes.map((r) => ({
    id: r.id, name: r.name,
    instruments: insts.filter(r.filter).map((i) => ({ id: i.id, code: i.code, type: i.type, typeLabel: TYPE_LABEL[i.type], sta: i.sta, offset: i.offset, unit: i.unit, mode: i.mode, tip_depth: i.tip_depth, zone_code: i.zone_code, recent: prev2.all(i.id) })),
  })));
}));

// ---------------------------------------------------------------- analisis

function settlementPayload(i: Instrument, q: any) {
  const opts = { dtDays: num(q.dt, 7), from: num(q.from), to: num(q.to) };
  const a = analyzeSettlement(i, opts);
  const z = getZone(i.zone_id!);
  const stages = getStages(z.id);
  ...
  const incsActual = loadIncrements(z, stages, getLayers(z.id));
  ...
  const incsPlan = loadIncrements(z, plannedStages, getLayers(z.id));
  ...
  const manualPair = (db().prepare(`SELECT id, code FROM instrument WHERE json_extract(meta, '$.pair') = ?`).all(i.code) as any[]);
  return {
    ...
    stages, design: { cv: z.cv, ch: z.ch, hdr: z.hdr, pvd: getPvd(z.id) },
    related: manualPair.map((m) => ({ code: m.code, data: dailySeries(m.id).map((p) => [p.t, +p.v.toFixed(1)]) })),
  };
}

api.get('/analysis/settlement/:id', wrap((req, res) => {
  const i = instGuard(req, res);
  if (!i) return;
  if (!['SC', 'GN', 'SP', 'SAA'].includes(i.type) || !i.zone_id) return res.status(400).json({ error: 'Bukan instrumen penurunan' });
  res.json(settlementPayload(i, req.query));
}));

api.get('/analysis/piezo/:id', wrap((req, res) => {
  const i = instGuard(req, res);
  if (!i) return;
  if (i.type !== 'PZ') return res.status(400).json({ error: 'Bukan piezometer' });
  const z = getZone(i.zone_id!);
  const stages = getStages(z.id);
  const ex = excessSeries(i, undefined, undefined, true);
  const rawRecent = excessSeries(i, Date.now() - 14 * DAY);
  ...
  const manual = db().prepare(`SELECT ts, value FROM reading WHERE instrument_id = ? AND source = 'manual' ORDER BY ts`).all(i.id) as any[];
  res.json({
    instrument: { ...i, meta: meta(i) }, zone: z, analysis: analyzePiezo(i),
...
api.post('/analysis/backanalysis/:id', requireRole('engineer'), wrap((req, res) => {
  const i = instGuard(req, res);
  if (!i) return;
  const z = getZone(i.zone_id!);
  const stages = getStages(z.id);
  const obs = rawSeries(i.id, num(req.body?.from), num(req.body?.to));
  const incs = loadIncrements(z, stages, getLayers(z.id));
  const r = backAnalyzeCh(obs, incs, consolidationParams(z), [0.3, 15]);
  const p = { ...consolidationParams(z), ch: r.ch };
...
api.post('/zones/:id/calibrate', requireRole('engineer'), wrap((req, res) => {
  const z = zoneGuard(req, res);
  if (!z) return;
  const ch = Number(req.body?.ch);
  if (!(ch > 0)) return res.status(400).json({ error: 'c_h tidak valid' });
  db().prepare('UPDATE zone SET ch = ? WHERE id = ?').run(ch, z.id);
  audit(req.user!.id, 'zone', z.id, 'kalibrasi c_h dari back-analysis', { ch: z.ch }, { ch, source: req.body?.source });
  res.json(getZone(z.id));
}));

/** Tabel perbandingan metode per zona (F-ANL-09). */
api.get('/zones/:id/compare', wrap((req, res) => {
  const z = zoneGuard(req, res);
  if (!z) return;
  const rows = zoneInstruments(z.id).filter((i) => ['SC', 'GN', 'SP', 'SAA'].includes(i.type)).map((i) => {
    const a = analyzeSettlement(i, { dtDays: num(req.query.dt, 7) });
    return {
...
api.post('/analysis/snapshots', requireRole('engineer'), wrap((req, res) => {
  const { instrument_id, zone_id, method, params, note } = req.body ?? {};
  let result: unknown;
  let inputRows: unknown[] = [];
  if (instrument_id) {
    const i = getInstrument(Number(instrument_id));
    if (!i || !projectGuard(req, res, i.project_id)) return;
    inputRows = db().prepare(`SELECT ts, value, source, flag FROM reading WHERE instrument_id = ? AND (flag IS NULL OR flag != 'ditolak') ORDER BY ts`).all(i.id);
    result = method === 'piezo' ? analyzePiezo(i) : analyzeSettlement(i, { dtDays: params?.dt, from: params?.from, to: params?.to });
  } else if (zone_id) {
    const z = getZone(Number(zone_id));
    if (!z || !projectGuard(req, res, z.project_id)) return;
    result = zoneStatus(z);
  ...
  ).run(instrument_id ?? null, zone_id ?? null, method ?? 'asaoka+hiperbolik', JSON.stringify(params ?? {}), input_hash, JSON.stringify(result), ALGO_VERSION, note ?? null, req.user!.id, Date.now()).lastInsertRowid;
  audit(req.user!.id, 'analysis_run', Number(id), 'simpan snapshot', null, { method, params, input_hash });
  res.json({ id, input_hash, algo_version: ALGO_VERSION });
}));

api.get('/analysis/snapshots', wrap((req, res) => {
  const rows = db().prepare(
...
api.get('/analysis/snapshots/:id/verify', wrap((req, res) => {
  const a = db().prepare('SELECT * FROM analysis_run WHERE id = ?').get(Number(req.params.id)) as any;
  if (!a) return res.status(404).json({ error: 'Snapshot tidak ditemukan' });
  const params = JSON.parse(a.params_json);
  const inputRows = a.instrument_id ? db().prepare(`SELECT ts, value, source, flag FROM reading WHERE instrument_id = ? AND (flag IS NULL OR flag != 'ditolak') AND received_at <= ? ORDER BY ts`).all(a.instrument_id, a.created_at) : [];
...
api.post('/zones/:id/scenario', wrap((req, res) => {
  const z = zoneGuard(req, res);
  if (!z) return;
  const stages = getStages(z.id).map((s) => ({ ...s }));
  ...
  const p = { ...consolidationParams(z), ch: ch ? Number(ch) : z.ch };
  const incs = loadIncrements(z, stages, layers);
  const base = loadIncrements(z, getStages(z.id).map((s) => ({ ...s, actual_start: s.actual_start ?? s.planned_start, actual_end: s.actual_end ?? (s.actual_start ? null : s.planned_end) })), layers);
  // skala ke data aktual as jalan
  const centre = zoneInstruments(z.id).filter((i) => ['SC', 'GN'].includes(i.type) && Math.abs(i.offset ?? 99) <= 6);
  const cur = centre.map((i) => lastReading(i.id)?.value).filter((v): v is number => v != null);
  const now = Date.now();
  ...
  for (let t = first; t <= end; t += 3 * DAY) {
    ...
    out.push({ t, base: stagedSettlement(t, base, p) * scale, scen: s, fillBase: fillHeightAt(getStages(z.id).map((x) => ({ ...x, actual_start: x.actual_start ?? x.planned_start, actual_end: x.actual_end ?? x.planned_end })), t), fillScen: fillHeightAt(stages, t) - (removal && t >= removal ? stages.filter((x) => x.is_surcharge).reduce((a, x) => a + x.thickness, 0) : 0) });
  }
...
api.get('/alarms', wrap((req, res) => {
  const status = String(req.query.status ?? 'open');
  const where = [
    status === 'open' ? '(e.cleared_at IS NULL OR e.ack_at IS NULL)' : status === 'unack' ? 'e.ack_at IS NULL' : '1=1',
    req.query.category ? 'e.category = @category' : '1=1',
    req.query.level ? 'e.level = @level' : '1=1',
    req.query.zone ? 'e.zone_id = @zone' : '1=1',
  ].join(' AND ');
  const rows = db().prepare(
  ...
  ).all({ category: req.query.category, level: req.query.level, zone: num(req.query.zone) });
  res.json(rows);
}));

api.post('/alarms/:id/ack', requireRole('engineer'), wrap((req, res) => {
  const note = String(req.body?.note ?? '').trim();
  if (note.length < 5) return res.status(400).json({ error: 'Catatan tindakan wajib diisi (min. 5 karakter)' });
  const e = db().prepare('SELECT * FROM alarm_event WHERE id = ?').get(Number(req.params.id)) as any;
  if (!e) return res.status(404).json({ error: 'Alarm tidak ditemukan' });
  db().prepare('UPDATE alarm_event SET ack_by = ?, ack_note = ?, ack_at = ? WHERE id = ?').run(req.user!.id, note, Date.now(), e.id);
  audit(req.user!.id, 'alarm_event', e.id, 'konfirmasi alarm', { ack_at: null }, { note });
  broadcast('alarms', { acked: e.id });
  res.json({ ok: true });
}));

api.get('/alarm-rules', wrap((req, res) => {
  const rows = db().prepare('SELECT r.*, z.code AS zone_code FROM alarm_rule r LEFT JOIN zone z ON z.id = r.zone_id ORDER BY parameter, zone_id, CASE level WHEN \'Waspada\' THEN 1 WHEN \'Siaga\' THEN 2 ELSE 3 END').all();
  res.json({ rules: rows, parameters: PARAMETERS });
}));

api.put('/alarm-rules/:id', requireRole('admin'), wrap((req, res) => {
  const r = db().prepare('SELECT * FROM alarm_rule WHERE id = ?').get(Number(req.params.id)) as any;
  ...
  db().prepare('UPDATE alarm_rule SET threshold = ?, enabled = ? WHERE id = ?').run(threshold, enabled, r.id);
  audit(req.user!.id, 'alarm_rule', r.id, 'ubah ambang', { threshold: r.threshold, enabled: r.enabled }, { threshold, enabled });
  res.json({ ok: true });
}));

api.post('/alarm-rules', requireRole('admin'), wrap((req, res) => {
  ...
  const z = zone_id ? getZone(Number(zone_id)) : null;
  const id = db().prepare('INSERT INTO alarm_rule(project_id, zone_id, parameter, kind, level, threshold) VALUES (?,?,?,?,?,?)')
    .run(z?.project_id ?? 1, zone_id ?? null, parameter, PARAMETERS[parameter].kind, level, Number(threshold)).lastInsertRowid;
  audit(req.user!.id, 'alarm_rule', Number(id), 'buat', null, req.body);
  res.json({ id });
}));

api.post('/alarms/reevaluate', requireRole('engineer'), wrap((req, res) => {
  const insts = db().prepare('SELECT * FROM instrument').all() as Instrument[];
  let n = 0;
  for (const i of insts) n += evaluateInstrument(i);
  res.json({ created: n });
}));

// ---------------------------------------------------------------- telemetri

api.get('/telemetry/devices', wrap((req, res) => {
  const d = db();
  const now = Date.now();
  const gws = d.prepare('SELECT id, code, vendor, model, serial, sta, power_type, last_seen FROM gateway').all() as any[];
  const lastH = d.prepare('SELECT * FROM device_health WHERE device_type = ? AND device_id = ? ORDER BY ts DESC LIMIT 1');
  const loggers = d.prepare('SELECT * FROM logger ORDER BY code').all() as any[];
  ...
  res.json({
    gateways: gws.map((g) => ({ ...g, health: lastH.get('gateway', g.id) ?? null, alarms: alarm.all('gateway', g.id) })),
    loggers: loggers.map((l) => {
      const ch = chans.all(l.id) as any[];
      let got = 0, expected = 0;
      for (const c of ch) { got += (cnt.get(c.id, now - DAY) as any).n; expected += (24 * 60) / c.expected_interval_min; }
      return { ...l, health: lastH.get('logger', l.id) ?? null, channels: ch, completeness24h: expected ? Math.min(1, got / expected) : null, alarms: alarm.all('logger', l.id) };
    }),
  });
}));

api.get('/telemetry/health/:type/:id', wrap((req, res) => {
  const rows = db().prepare('SELECT ts, battery_v, battery_pct, rssi, snr, internal_temp FROM device_health WHERE device_type = ? AND device_id = ? AND ts > ? ORDER BY ts')
...
api.get('/telemetry/compare', wrap((req, res) => {
  const d = db();
  const pairs: any[] = [];
  ...
  const manualInst = d.prepare(`SELECT * FROM instrument WHERE type = 'SP'`).all() as Instrument[];
  for (const m of manualInst) {
    const ref = d.prepare('SELECT * FROM instrument WHERE code = ?').get(meta(m).pair) as Instrument;
    if (!ref) continue;
    const rows = d.prepare(`SELECT ts, value FROM reading WHERE instrument_id = ? AND source = 'manual' ORDER BY ts`).all(m.id) as any[];
    const data = rows.map((r) => { const t = nearest.get(ref.id, r.ts - 3 * 3600e3, r.ts + 3 * 3600e3, r.ts) as any; return t ? { ts: r.ts, manual: r.value, telemetry: t.value, diff: t.value - r.value } : null; }).filter(Boolean);
    pairs.push({ kind: 'penurunan', manual: m.code, telemetry: ref.code, unit: 'mm', tolerance: 5, data });
  }
  const pz = d.prepare(`SELECT DISTINCT i.* FROM instrument i JOIN reading r ON r.instrument_id = i.id WHERE i.type = 'PZ' AND r.source = 'manual'`).all() as Instrument[];
  for (const p of pz) {
    const rows = d.prepare(`SELECT ts, value FROM reading WHERE instrument_id = ? AND source = 'manual' ORDER BY ts`).all(p.id) as any[];
    const data = rows.map((r) => { const t = nearest.get(p.id, r.ts - 3 * 3600e3, r.ts + 3 * 3600e3, r.ts) as any; return t ? { ts: r.ts, manual: r.value, telemetry: t.value, diff: t.value - r.value } : null; }).filter(Boolean);
    pairs.push({ kind: 'tekanan pori', manual: `${p.code} (readout)`, telemetry: p.code, unit: 'kPa', tolerance: 2, data });
  }
  const gns = d.prepare(`SELECT * FROM instrument WHERE type = 'GN' AND json_extract(meta, '$.pair') IS NOT NULL`).all() as Instrument[];
  for (const g of gns) {
    const ref = d.prepare('SELECT * FROM instrument WHERE code = ?').get(meta(g).pair) as Instrument;
    const rows = d.prepare(`SELECT ts, value FROM reading WHERE instrument_id = ? AND ts > ? AND CAST((ts + 25200000) / 3600000 AS INTEGER) % 24 = 12 ORDER BY ts`).all(g.id, Date.now() - 60 * DAY) as any[];
    const data = rows.map((r) => { const t = nearest.get(ref.id, r.ts - 3 * 3600e3, r.ts + 3 * 3600e3, r.ts) as any; return t ? { ts: r.ts, manual: r.value, telemetry: t.value, diff: t.value - r.value } : null; }).filter(Boolean);
...
api.get('/telemetry/maintenance', wrap((req, res) => {
  res.json(db().prepare(
...
api.post('/telemetry/maintenance', requireRole('surveyor'), wrap((req, res) => {
  const { device_code, action, note, ts } = req.body ?? {};
  const d = db();
  const found = (d.prepare(`SELECT 'logger' t, id FROM logger WHERE code = ? UNION SELECT 'gateway', id FROM gateway WHERE code = ? UNION SELECT 'instrument', id FROM instrument WHERE code = ?`).get(device_code, device_code, device_code)) as any;
  if (!found || !action) return res.status(400).json({ error: 'Perangkat dan tindakan wajib diisi' });
  const id = d.prepare('INSERT INTO maintenance_log(device_type, device_id, ts, action, note, by_user) VALUES (?,?,?,?,?,?)').run(found.t, found.id, ts ? Number(ts) : Date.now(), action, note ?? null, req.user!.id).lastInsertRowid;
  audit(req.user!.id, 'maintenance_log', Number(id), 'buat', null, req.body);
  res.json({ id });
}));

api.get('/telemetry/reference-checks', wrap((req, res) => {
  res.json(db().prepare('SELECT r.*, u.name AS by_name FROM reference_check r LEFT JOIN app_user u ON u.id = r.by_user ORDER BY ts DESC').all());
}));

api.post('/telemetry/reference-checks', requireRole('surveyor'), wrap((req, res) => {
  const { target = 'TANK-01', elevation, ts, method } = req.body ?? {};
  const first = db().prepare('SELECT elevation FROM reference_check WHERE target = ? ORDER BY ts LIMIT 1').get(target) as any;
  ...
  const id = db().prepare('INSERT INTO reference_check(target, ts, elevation, delta_mm, method, by_user) VALUES (?,?,?,?,?,?)').run(target, ts ? Number(ts) : Date.now(), e, delta, method ?? 'Waterpass', req.user!.id).lastInsertRowid;
  audit(req.user!.id, 'reference_check', Number(id), 'buat', null, { target, elevation: e, delta });
  res.json({ id, delta_mm: delta });
}));

// ---------------------------------------------------------------- laporan, ekspor, audit, pengguna

api.get('/reports/weekly', wrap((req, res) => {
  const pid = num(req.query.project, 1)!;
  if (!projectGuard(req, res, pid)) return;
  const to = num(req.query.to, Date.now())!;
  const from = num(req.query.from, to - 7 * DAY)!;
  res.type('html').send(weeklyReport(pid, from, to, String(req.query.lang ?? 'id') as 'id' | 'en', req.user!));
}));

const csvCell = (v: unknown) => (v == null ? '' : /[",;\n]/.test(String(v)) ? `"${String(v).replace(/"/g, '""')}"` : String(v));
const iso = (t: number) => new Date(t + 7 * 3600e3).toISOString().replace('Z', '+07:00').replace('.000', '');

api.get('/export/readings.csv', wrap((req, res) => {
  const pid = num(req.query.project, 1)!;
  if (!projectGuard(req, res, pid)) return;
  const from = num(req.query.from, 0)!;
  const to = num(req.query.to, Date.now())!;
...
api.get('/export/analysis.csv', wrap((req, res) => {
  const pid = num(req.query.project, 1)!;
  if (!projectGuard(req, res, pid)) return;
  ...
  for (const z of getZones(pid)) {
    for (const i of zoneInstruments(z.id).filter((x) => ['SC', 'GN', 'SP', 'SAA'].includes(x.type))) {
      const a = analyzeSettlement(i);
...
api.get('/audit', requireRole('engineer'), wrap((req, res) => {
  res.json(db().prepare('SELECT a.*, u.name AS user_name FROM audit_log a LEFT JOIN app_user u ON u.id = a.user_id ORDER BY ts DESC LIMIT ?').all(num(req.query.limit, 200)));
}));

api.get('/users', requireRole('admin'), wrap((req, res) => {
  res.json(db().prepare('SELECT id, email, name, role FROM app_user ORDER BY id').all());
}));

api.put('/users/:id', requireRole('admin'), wrap((req, res) => {
  const u = db().prepare('SELECT id, role FROM app_user WHERE id = ?').get(Number(req.params.id)) as any;
  ...
  db().prepare('UPDATE app_user SET role = ? WHERE id = ?').run(role, u.id);
  audit(req.user!.id, 'app_user', u.id, 'ubah role', { role: u.role }, { role });
  res.json({ ok: true });
}));
