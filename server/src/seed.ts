...
function main() {
  ...
  for (const [email, name, role] of users) db.prepare('INSERT INTO app_user(email, name, role, password_hash) VALUES (?,?,?,?)').run(email, name, role, pw);
  ...
  ).run('Jalan Tol Pesisir — Paket Timbunan Tanah Lunak STA 24+000 – 25+000', 'TPS-P3', 'jalan', 32749, 'MSL lokal (BM-07)', 'Asia/Jakarta', 'WIB', now).lastInsertRowid);
  for (let u = 1; u <= 5; u++) db.prepare('INSERT INTO project_member(project_id, user_id, role) VALUES (?,?,?)').run(pid, u, users[u - 1][2]);
  const geom = [];
  const prof = [];
  for (let s = 24000; s <= 25000; s += 25) { geom.push({ sta: s, ...staToXY(s) }); prof.push({ sta: s, elev: +designElevation(s).toFixed(3) }); }
  db.prepare('INSERT INTO alignment(project_id, name, sta_geometry, design_profile) VALUES (?,?,?,?)').run(pid, 'As jalan utama', JSON.stringify(geom), JSON.stringify(prof));
  ...
  for (const z of ZONES) {
    ...
    const id = Number(db.prepare(
      `INSERT INTO zone(project_id, code, name, sta_start, sta_end, polygon, is_transition, design_fill_height, surcharge_height, ground_elev, crest_width, slope_h, cv, ch, hdr, residual_limit_mm)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    ).run(pid, z.code, z.name, z.staStart, z.staEnd, JSON.stringify([a, b, c, d]), z.transition ? 1 : 0, z.designH, z.surchargeH, GROUND_ELEV, 24, 2, DESIGN.cv, DESIGN.ch, DESIGN.Hdr, 100).lastInsertRowid);
    zoneId[z.code] = id;
    ...
    db.prepare('INSERT INTO pvd_spec(zone_id, pattern, spacing, length, a, b, s, kh_ks) VALUES (?,?,?,?,?,?,?,?)').run(id, PVD.pattern, PVD.spacing, PVD.length, PVD.a, PVD.b, PVD.s, PVD.khKs);
  ...
  const gwId = Number(db.prepare(
    `INSERT INTO gateway(project_id, code, vendor, model, serial, x, y, sta, power_type, token_hash, last_seen) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
  ).run(pid, 'GW-01', 'Vendor pilot', 'Gateway LoRa AS923-2 + 4G', 'GW2600411', gw.x, gw.y, 24500, 'Panel surya 100 Wp + baterai LiFePO4 100 Ah', hashToken(GATEWAY_TOKEN), now).lastInsertRowid);

  const { instruments, loggers } = buildLayout();
  const loggerId: Record<string, number> = {};
  const loggerInstall: Record<string, number> = {};
  for (const l of loggers) {
    loggerId[l.code] = Number(db.prepare(
      `INSERT INTO logger(gateway_id, code, vendor, model, serial, firmware, sta, offset, channels, last_seen) VALUES (?,?,?,?,?,?,?,?,?,?)`,
    ).run(gwId, l.code, l.vendor, l.model, l.serial, l.firmware, l.sta, l.offset, l.channels, null).lastInsertRowid);
  }
  ...
  for (const i of instruments) {
    ...
    const id = Number(db.prepare(
      ...
      i.type === 'PZ' ? i.meta.u_hydro : 0, i.unit, i.mode, 'aktif', JSON.stringify(i.meta), i.intervalMin).lastInsertRowid);
    instId[i.code] = id;
    if (i.logger) {
      db.prepare('INSERT INTO logger_channel(logger_id, channel_no, instrument_id, valid_from) VALUES (?,?,?,?)').run(loggerId[i.logger], i.channel, id, i.installedAt);
      loggerInstall[i.logger] = Math.min(loggerInstall[i.logger] ?? Infinity, i.installedAt);
    }
    const cal = calibrationFor(i);
    if (cal) {
      db.prepare('INSERT INTO calibration(instrument_id, valid_from, coeffs_json, certificate_file, next_due) VALUES (?,?,?,?,?)')
        .run(id, i.installedAt - 30 * DAY, JSON.stringify(cal), `sertifikat/${i.code}.pdf`, i.installedAt + 335 * DAY);
    }
  }
  for (const c of TANK_CHECKS) {
    db.prepare('INSERT INTO reference_check(target, ts, elevation, delta_mm, method, by_user) VALUES (?,?,?,?,?,?)')
      .run('TANK-01', Date.parse(c.date + 'T09:00:00+07:00'), +(GROUND_ELEV + 1.2 - c.delta / 1000).toFixed(4), c.delta, 'Waterpass dari BM-07', 3);
  }

  // ---- riwayat bacaan
  const ins = db.prepare(
    `INSERT OR IGNORE INTO reading(instrument_id, ts, raw_value, raw_temp, value, source, calib_id, flag, note, entered_by, received_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
  );
  const insP = db.prepare('INSERT OR IGNORE INTO reading_profile(instrument_id, ts, pos, value) VALUES (?,?,?,?)');
  ...
  const tx = db.transaction(() => {
    for (const i of instruments) {
      ...
      const cid = (calId.get(id) as any)?.id ?? null;
      if (i.mode === 'manual') {
        ...
        for (; t <= now; t += 7 * DAY) {
          const v = truth(i, t);
          if (!v) continue;
          ins.run(id, t, null, null, +v.value.toFixed(0), 'manual', null, null, null, 3, t + 6 * HOUR);
          nReadings++;
        }
        continue;
      }
      ...
      for (let t = start; t <= now; t += step(t)) {
        ...
        ins.run(id, t, raw?.raw ?? null, raw?.temp ?? null, v.value, 'telemetry', raw ? cid : null, null, null, null, t + 2 * 60e3);
        nReadings++;
        if (v.profile && (t >= now - 21 * DAY || (t / HOUR) % 24 === 5)) for (const p of v.profile) insP.run(id, t, p.pos, p.value);
      }
      ...
      if (i.type === 'PZ' && i.tipDepth === 9 && i.offset === 0 && [24200, 24500, 24800].includes(i.sectionSta)) {
        for (let t = i.installedAt + 7 * DAY + 2 * HOUR; t <= Math.min(now, i.installedAt + 120 * DAY); t += 7 * DAY) {
          const v = truth(i, t)!;
          ins.run(id, t, null, null, +(v.value + noise(i.code + 'M', t, 0.9)).toFixed(1), 'manual', null, null, 'Readout portabel', 3, t + 5 * HOUR);
          nReadings++;
        }
      }
    }
  ...
  // ---- kesehatan perangkat
  const insH = db.prepare(`INSERT OR REPLACE INTO device_health(device_type, device_id, ts, battery_v, battery_pct, rssi, snr, internal_temp) VALUES (?,?,?,?,?,?,?,?)`);
  db.transaction(() => {
    for (const l of loggers) {
      ...
      for (let t = Math.ceil(inst0 / HOUR) * HOUR; t <= now; t += t < now - 3 * DAY ? 6 * HOUR : HOUR) {
        ...
        insH.run('logger', loggerId[l.code], t, +(3.3 + 0.35 * pct / 100).toFixed(3), +pct.toFixed(1), +(rssiFor(l) + noise(l.code + 'r', t, 2)).toFixed(0), +(8 + noise(l.code + 's', t, 1.5)).toFixed(1), +sensorTemp(l.code, t).toFixed(1));
        last = t;
      }
      db.prepare('UPDATE logger SET last_seen = ? WHERE id = ?').run(last || null, loggerId[l.code]);
    }
    for (let t = now - 60 * DAY; t <= now; t += 6 * HOUR) {
      const h = ((t / HOUR) + 7) % 24;
      const pct = 78 + 18 * Math.max(0, Math.sin((Math.PI * (h - 6)) / 12));
      insH.run('gateway', gwId, t, +(12.4 + pct / 100).toFixed(2), +pct.toFixed(0), -71, 12, 36);
    }
  })();
  ...
  for (const [p, w, s, b] of rules) {
    for (const [lvl, th] of [['Waspada', w], ['Siaga', s], ['Bahaya', b]] as const) {
      db.prepare('INSERT INTO alarm_rule(project_id, zone_id, parameter, kind, level, threshold, window_h) VALUES (?,?,?,?,?,?,?)')
  ...
  const all = db.prepare('SELECT id FROM instrument').all() as { id: number }[];
  for (let t = now - 45 * DAY; t <= now; t += t < now - 3 * DAY ? 12 * HOUR : HOUR) {
    for (const { id } of all) evaluateInstrument(getInstrument(id), t);
  }
  evaluateDevices(now);
  const notes = [
  ...
  const old = db.prepare(`SELECT id, ts FROM alarm_event WHERE category = 'geoteknik' AND ts < ?`).all(now - 5 * DAY) as { id: number; ts: number }[];
  old.forEach((e, k) => db.prepare('UPDATE alarm_event SET ack_by = 2, ack_note = ?, ack_at = ? WHERE id = ?').run(notes[k % notes.length], e.ts + (2 + (k % 5)) * HOUR, e.id));
  ...
  for (const [t, c, dt, a, note] of mlog) {
    ...
    db.prepare('INSERT INTO maintenance_log(device_type, device_id, ts, action, note, by_user) VALUES (?,?,?,?,?,?)').run(t, id, Date.parse(dt + 'T10:00:00+07:00'), a, note, 4);
  }
  db.prepare('INSERT INTO recommendation(zone_id, decision, text, created_by, created_at) VALUES (?,?,?,?,?)')
    .run(zoneId['Z-02'], 'Tahan', 'Surcharge dipertahankan. U Asaoka belum mencapai 90%; evaluasi ulang dua minggu lagi.', 2, now - 6 * DAY);

  const count = (db.prepare('SELECT COUNT(*) n FROM reading').get() as any).n;
  const alarms = db.prepare('SELECT category, level, COUNT(*) n FROM alarm_event WHERE cleared_at IS NULL GROUP BY category, level').all();
