...
export function computeMetric(inst: Instrument, parameter: string, at: number): Metric {
  const db = getDb();
  switch (parameter) {
    case 'lateral_rate':
    case 'toe_h_rate': {
      const s = rawSeries(inst.id, at - 2 * DAY, at);
      const r = rate(s, DAY, at);
      return Number.isFinite(r) ? { value: r } : null;
    }
    case 'settlement_rate': {
      if (Math.abs(inst.offset ?? 99) > 6) return null;
      const stages = getStages(inst.zone_id!);
      ...
      const r = rate(rawSeries(inst.id, at - 2 * DAY, at), DAY, at);
      return Number.isFinite(r) ? { value: r } : null;
    }
    case 'du_dsigma': {
      const stages = getStages(inst.zone_id!);
      ...
      const before = valueAt(inst.id, cur.actual_start!);
      const now = valueAt(inst.id, at);
      if (before == null || now == null) return null;
      return { value: (now - before) / dS, detail: `Δσ = ${dS.toFixed(0)} kPa, tahap ${cur.stage_no}` };
    }
    case 'delta_over_s': {
      const d = valueAt(inst.id, at);
      const m = meta(inst);
      if (d == null || !m.pair_settlement) return null;
      const ref = db.prepare('SELECT id FROM instrument WHERE code = ?').get(m.pair_settlement) as any;
      if (!ref) return null;
      const S = valueAt(ref.id, at);
...
function rulesFor(inst: Instrument): Rule[] {
 ...
 */
export function evaluateInstrument(inst: Instrument, at = Date.now()): number {
  const db = getDb();
  const rules = rulesFor(inst);
  if (!rules.length) return 0;
  const prev = db.prepare('SELECT ts FROM reading WHERE instrument_id = ? AND ts <= ? ORDER BY ts DESC LIMIT 2').all(inst.id, at) as { ts: number }[];
  ...
  for (const [param, rs] of byParam) {
    const m1 = computeMetric(inst, param, prev[0].ts);
    const m2 = computeMetric(inst, param, prev[1].ts);
    ...
    if (eff && m1) {
      if (!open || rank(eff.level) > rank(open.level)) {
        if (open) db.prepare('UPDATE alarm_event SET cleared_at = ? WHERE id = ?').run(prev[0].ts, open.id);
        ...
        db.prepare('UPDATE alarm_event SET value = MAX(value, ?) WHERE id = ?').run(m1.value, open.id);
      }
    } else if (open && !l1 && !l2) {
      db.prepare('UPDATE alarm_event SET cleared_at = ? WHERE id = ?').run(prev[0].ts, open.id);
    }
  }
  return created;
}
...
function raiseTechnical(parameter: string, deviceType: string, deviceId: number, level: Level, message: string, value: number | null, ts: number, instrumentId: number | null = null, zoneId: number | null = null) {
  const db = getDb();
  const open = db.prepare(
...
function clearTechnical(parameter: string, deviceType: string, deviceId: number, ts: number) {
  getDb().prepare(
...
export function evaluateDevices(now = Date.now()) {
  const db = getDb();
  const devices = [
    ...(db.prepare(`SELECT id, code, last_seen, 'logger' AS t FROM logger`).all() as any[]),
    ...(db.prepare(`SELECT id, code, last_seen, 'gateway' AS t FROM gateway`).all() as any[]),
  ];
  for (const d of devices) {
    const hrs = d.last_seen ? (now - d.last_seen) / 3600e3 : Infinity;
    if (hrs > OFFLINE_HOURS) {
      raiseTechnical('offline', d.t, d.id, hrs > 24 ? 'Siaga' : 'Waspada', `${d.t === 'logger' ? 'Logger' : 'Gateway'} ${d.code} tidak mengirim data ${Number.isFinite(hrs) ? hrs.toFixed(0) + ' jam' : 'sejak dipasang'}`, hrs, now);
    } else clearTechnical('offline', d.t, d.id, now);
    const h = db.prepare(`SELECT battery_pct FROM device_health WHERE device_type = ? AND device_id = ? ORDER BY ts DESC LIMIT 1`).get(d.t, d.id) as any;
    if (h?.battery_pct != null && h.battery_pct < BATTERY_MIN_PCT) {
      raiseTechnical('battery_low', d.t, d.id, h.battery_pct < 10 ? 'Siaga' : 'Waspada', `Baterai ${d.code} ${h.battery_pct.toFixed(0)}% (< ${BATTERY_MIN_PCT}%)`, h.battery_pct, now);
    } else if (h) clearTechnical('battery_low', d.t, d.id, now);
  }
}

export function raiseReadingAlarm(inst: Instrument, kind: 'di_luar_rentang' | 'lonjakan', value: number, ts: number, message: string) {
  raiseTechnical(kind, 'instrument', inst.id, 'Waspada', message, value, ts, inst.id, inst.zone_id);
}

/** Validasi lonjakan (F-INS-02): |v − rerata| > n × simpangan baku dari 20 pembacaan terakhir, disesuaikan tren. */
export function spikeCheck(instId: number, ts: number, value: number, n = 4): { spike: boolean; expected: number | null; sd: number | null } {
  const rows = getDb().prepare('SELECT ts AS t, value AS v FROM reading WHERE instrument_id = ? AND ts < ? ORDER BY ts DESC LIMIT 20').all(instId, ts) as Point[];
