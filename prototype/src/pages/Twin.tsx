...
function polyFor(geom: { sta: number; x: number; y: number }[], s0: number, s1: number) {
  // poligon koridor ±30 m mengikuti lengkung trase (normal per segmen)
  const pts = geom.filter((p) => p.sta >= s0 && p.sta <= s1);
  if (pts.length < 2) return [];
  const off = (k: number, d: number) => {
    const a = pts[Math.max(0, k - 1)], b = pts[Math.min(pts.length - 1, k + 1)];
    const L = Math.hypot(b.x - a.x, b.y - a.y) || 1;
    const nx = (b.y - a.y) / L, ny = -(b.x - a.x) / L;
    return { x: pts[k].x + d * nx, y: pts[k].y + d * ny };
  };
  const left = pts.map((_, k) => off(k, -30));
  const right = pts.map((_, k) => off(k, 30)).reverse();
  return [...left, ...right];
}
